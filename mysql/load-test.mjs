...
async function main() {
  console.log(
    `Teste de carga: ${REQUESTS} requisições, concorrência ${CONCURRENCY}, pool connectionLimit=${CONNECTION_LIMIT}`,
  );
  console.log(
    `Alvo: ${process.env.MYSQL_HOST ?? "localhost"}:${process.env.MYSQL_PORT ?? 3306}/${process.env.MYSQL_DATABASE ?? "(não definido)"}`,
  );
  ...
  console.log(
    `Latência por requisição — p50: ${p50.toFixed(1)}ms | p95: ${p95.toFixed(1)}ms | máx: ${max.toFixed(1)}ms`,
  );
  if (falha.length > 0) {
