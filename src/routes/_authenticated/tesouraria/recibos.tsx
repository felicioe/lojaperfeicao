...
async function imprimirRecibo(r: {
  numero: number;
  tipo: string;
  data: string;
  valor: number;
  ...
  // Dados institucionais da Loja (issue #340) — antes eram 3 logos e uma
  // razão social fixos no HTML, corretos só para a instalação da Adonhiram.
  const [loja, logos] = await Promise.all([obterLojaAtual(), listarLogosInstitucionais()]);
  const nomeEntidade = (loja.razaoSocial || loja.nome).toUpperCase();
  const logosHtml = logos.map((l) => `<img src="${l.logoUrl}" alt="${l.nome}">`).join("");

  janela.document.write(
    // A barra em `<\/script>` dentro do template é obrigatória: sem ela o
    ...
    `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Recibo ${r.numero}</title><style>:root{--ink:#172033;--muted:#5b6472;--gold:#9b7b32}body{font-family:Georgia,"Times New Roman",serif;color:var(--ink);padding:36px;max-width:820px;margin:auto}.head{display:flex;align-items:center;justify-content:space-between;gap:16px;border-bottom:2px solid var(--gold);padding-bottom:16px}.logos{display:flex;flex-wrap:wrap;gap:8px}.logos img{width:60px;height:60px;object-fit:contain}.center{flex:1;text-align:center;font-size:13px;font-weight:bold}.title{text-align:center;margin:36px 0 24px}.valor{border:1px solid var(--muted);padding:10px 16px;float:right;font-size:20px}.texto{font-size:17px;line-height:1.8;clear:both;padding-top:25px}.assinatura{margin-top:80px;text-align:center}.linha{border-top:1px solid var(--ink);width:360px;margin:auto;padding-top:8px}@media print{button{display:none}}</style></head><body><div class="head"><div class="logos">${logosHtml}</div><div class="center">${nomeEntidade}${loja.cnpj ? `<br>CNPJ ${loja.cnpj}` : ""}</div></div><h1 class="title">RECIBO Nº ${r.numero}</h1><div class="valor">${brl(r.valor)}</div><p class="texto">${r.tipo === "recebimento" ? "Recebemos de" : "Pagamos a"} <strong>${r.pessoa_nome}</strong> a importância de <strong>${valorPorExtenso(Number(r.valor))}</strong>, referente a ${r.descricao}.${r.forma_pagamento ? `<br>Forma de pagamento: ${r.forma_pagamento}.` : ""}</p>${r.observacoes ? `<p>${r.observacoes}</p>` : ""}<p>Camboriú, ${fmtDate(r.data)}.</p><div class="assinatura"><div class="linha">${nomeEntidade}</div></div><script>setTimeout(()=>window.print(),500)<\/script></body></html>`,
  );
