...
import { listarPotencias, salvarPotencia, alternarAtivoPotencia, type Potencia } from "@/lib/server/orgs";
...
function Potencias() {
  ...
  return (
    <>
      <PageHeader title="Potência" description="Órgãos federativos/obediências aos quais os corpos maçônicos são filiados." />

      {can.canManageIrmaos && (
        <Card className="mb-4">
          <CardHeader><CardTitle className="text-base">{form.id ? "Editar potência" : "Nova potência"}</CardTitle></CardHeader>
          <CardContent className="grid gap-3 md:grid-cols-4">
            <div><Label>Nome</Label><Input value={form.nome} onChange={(e) => setForm({ ...form, nome: e.target.value })} /></div>
            <div><Label>Sigla</Label><Input value={form.sigla} onChange={(e) => setForm({ ...form, sigla: e.target.value })} /></div>
            <div><Label>Jurisdição</Label><Input value={form.jurisdicao} onChange={(e) => setForm({ ...form, jurisdicao: e.target.value })} /></div>
      ...
      <Card>
        <Table>
          ...
          <TableBody>
            {data.length === 0 && (
              <TableRow><TableCell colSpan={5} className="text-center py-6 text-muted-foreground">Nenhuma potência cadastrada.</TableCell></TableRow>
            )}
            {data.map((p) => (
              <TableRow key={p.id} className={!p.ativo ? "opacity-50" : undefined}>
                ...
                <TableCell><Switch checked={p.ativo} onCheckedChange={() => alternarAtivo(p)} disabled={!can.canManageIrmaos} /></TableCell>
                {can.canManageIrmaos && (
                  <TableCell className="text-right">
                    <Button variant="ghost" size="sm" onClick={() => setForm({ id: p.id, nome: p.nome, sigla: p.sigla ?? "", jurisdicao: p.jurisdicao ?? "", site: p.site ?? "" })}>
