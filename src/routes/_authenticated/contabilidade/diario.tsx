...
import { listarLancamentosDiario } from "@/lib/server/contabilidade";
  ...
  return (
    <>
      ...
      <Card className="mb-4 p-4 grid gap-3 md:grid-cols-3">
        ...
        <div className="flex flex-col justify-end"><div className="text-sm text-muted-foreground">Total de débitos no período</div><div className="text-xl font-semibold">{brl(totalGeral)}</div></div>
      </Card>

      <div className="space-y-3">
        {lancamentos.length === 0 && <Card className="p-6 text-center text-muted-foreground">Nenhum lançamento no período.</Card>}
        {lancamentos.map((l) => (
          <Card key={l.id}>
            <div className="flex items-center justify-between p-3 border-b">
              <div>
                <div className="font-medium">{l.descricao}</div>
                <div className="text-xs text-muted-foreground">{fmtDate(l.data)}{l.origem_tipo ? ` · ${l.origem_tipo}` : ""}</div>
              </div>
            </div>
            <Table>
              <TableHeader>
                <TableRow><TableHead>Conta</TableHead><TableHead className="text-right">Débito</TableHead><TableHead className="text-right">Crédito</TableHead></TableRow>
              </TableHeader>
              <TableBody>
                {(l.lancamentos_contabeis_itens ?? []).map((it: any) => (
                  <TableRow key={it.id}>
                    <TableCell>{it.plano_contas?.codigo} — {it.plano_contas?.nome}</TableCell>
                    <TableCell className="text-right">{it.tipo === "debito" ? brl(it.valor) : ""}</TableCell>
                    <TableCell className="text-right">{it.tipo === "credito" ? brl(it.valor) : ""}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
