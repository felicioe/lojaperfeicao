...
import { concluirLoginGoogle } from "@/lib/backend/google-auth";
import { SESSAO_QUERY_KEY } from "@/lib/auth-hooks";
import { Card, CardContent } from "@/components/ui/card";
import { toast } from "sonner";

export const Route = createFileRoute("/google-concluir")({
  head: () => ({ meta: [{ title: "Entrando… — Gestão Maçônica" }] }),
  component: GoogleConcluirPage,
});

function GoogleConcluirPage() {
    ...
    <div className="flex min-h-screen items-center justify-center bg-muted/30 p-4">
      <Card className="w-full max-w-sm">
        <CardContent className="py-8 text-center text-sm text-muted-foreground">
          {erro ? erro : "Entrando com sua conta Google…"}
        </CardContent>
      </Card>
    </div>
  );
